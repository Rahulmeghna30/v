...
# ========== CONFIGURATION ==========
TOKEN = "8545477292:AAGa_lzqo1mvc-Oi0v8LIXo1wz2dWVK_ix8"

# Multiple Owners
OWNER_IDS = [8000588677, 8495511397]

# Multiple Admins
ADMIN_IDS = [8000588677, 8495511397]

YOUR_USERNAME = "@Its_MeVisha"
...
REFERRAL_REQUIRED = 5  # 5 invites = +1 bot slot
...
# ========== MISSING FUNCTION DEFINITIONS (MOVE TO TOP) ==========
...
3. When 5 friends join → Get +1 bot slot!
...
def create_admin_panel():
    ...
    markup.row(types.InlineKeyboardButton('📋 ʟɪꜱᴛ ᴀᴅᴍɪɴꜱ', callback_data='list_admins'))
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        logger.error(f"⚠️ Error processing zip: {e}", exc_info=True)
...
def handle_js_file(file_path, script_owner_id, user_folder, file_name, message):
    try:
        ...
        logger.error(f"⚠️ Error processing JS file: {e}", exc_info=True)
...
def handle_py_file(file_path, script_owner_id, user_folder, file_name, message):
    try:
        ...
        logger.error(f"⚠️ Error processing Python file: {e}", exc_info=True)
...
BUTTON_TEXT_TO_LOGIC = {
    ...
    # New buttons
    "🎯 ʀᴇꜰᴇʀʀᴀʟ ꜱʏꜱᴛᴇᴍ": referral_command,
    ...
    if user_id not in active_users:
        ...
        try:
            owner_notification = (
                f"👤 ɴᴇᴡ ᴜꜱᴇʀ!\n"
                f"📛 ɴᴀᴍᴇ: {user_name}\n"
                f"👤 ᴜꜱᴇʀɴᴀᴍᴇ: @{message.from_user.username or 'N/A'}\n"
                f"🆔 ɪᴅ: `{user_id}`"
            )
...
# Add these missing function definitions before they're used:
...
# ========== BUTTON TEXT TO LOGIC MAPPING ==========
# Replace the entire BUTTON_TEXT_TO_LOGIC section with:
...
def _logic_bot_speed(message):
    ...
    try:
        ...
        # Count running bots safely
        running_bots = sum(
            1 for k in list(bot_scripts.keys())
            if is_bot_running(int(k.split('_')[0]), k.split('_', 1)[1])
        )
        ...
        speed_msg = (
            "⚡ ʙᴏᴛ ꜱᴘᴇᴇᴅ & ꜱᴛᴀᴛᴜꜱ\n\n"
            "📊 ᴘᴇʀꜰᴏʀᴍᴀɴᴄᴇ:\n"
            f"• ⏱️ ʀᴇꜱᴘᴏɴꜱᴇ ᴛɪᴍᴇ: {response_time} ms\n"
            f"• 🔧 ꜱᴛᴀᴛᴜꜱ: {status}\n"
            f"• 👤 ʏᴏᴜʀ ʟᴇᴠᴇʟ: {user_level}\n\n"
            "🌐 ꜱʏꜱᴛᴇᴍ:\n"
            f"• 🤖 ʀᴜɴɴɪɴɢ ʙᴏᴛꜱ: {running_bots}\n"
            f"• 👥 ᴀᴄᴛɪᴠᴇ ᴜꜱᴇʀꜱ: {len(active_users)}\n"
            f"• 📦 ᴛᴏᴛᴀʟ ꜰɪʟᴇꜱ: {sum(len(files) for files in user_files.values())}\n\n"
            "🚀 ᴏᴛʜᴇʀ ꜱᴛᴀᴛꜱ:\n"
            f"• 💾 ʏᴏᴜʀ ꜰɪʟᴇꜱ: {len(user_files.get(user_id, []))}\n"
            f"• 🕒 ᴜᴘᴛɪᴍᴇ: {uptime_sec} s"
        )

        # Try editing message, fallback to send
        try:
            bot.edit_message_text(speed_msg, chat_id, wait_msg.message_id)
        except Exception:
            bot.send_message(chat_id, speed_msg)
        ...
        try:
            bot.edit_message_text(
                "❌ ꜱᴘᴇᴇᴅ ᴛᴇꜱᴛ ꜰᴀɪʟᴇᴅ. ᴘʟᴇᴀꜱᴇ ᴛʀʏ ᴀɢᴀɪɴ.",
                chat_id,
                wait_msg.message_id
            )
        except Exception:
...
def _logic_ai_assistant(message):
    bot.reply_to(message, "🤖 ᴀᴛx ᴀɪ ᴀꜱꜱɪꜱᴛᴀɴᴛ\n\nɪ'ᴍ ʜᴇʀᴇ ᴛᴏ ʜᴇʟᴘ ʏᴏᴜ ᴡɪᴛʜ:\n• ᴄᴏᴅɪɴɢ Qᴜᴇꜱᴛɪᴏɴꜱ\n• ʙᴏᴛ ᴅᴇᴠᴇʟᴏᴘᴍᴇɴᴛ\n• ᴛʀᴏᴜʙʟᴇꜱʜᴏᴏᴛɪɴɢ\n• ɢᴇɴᴇʀᴀʟ ɢᴜɪᴅᴀɴᴄᴇ\n\nꜱᴇʟᴇᴄᴛ ᴀɴ ᴏᴘᴛɪᴏɴ ʙᴇʟᴏᴡ ᴏʀ ᴛʏᴘᴇ ʏᴏᴜʀ Qᴜᴇꜱᴛɪᴏɴ:", reply_markup=create_ai_assistant_menu())
...
"""
    ...
    if total_users > 5:
        users_list_msg += f"\n📄 ꜱʜᴏᴡɪɴɢ 5/10 ʀᴇᴄᴇɴᴛ ᴜꜱᴇʀꜱ. ᴜꜱᴇ /ᴇxᴘᴏʀᴛ ᴛᴏ ɢᴇᴛ ꜰᴜʟʟ ʟɪꜱᴛ."
...
def handle_file_upload_doc(message):
    ...
    try:
        try:
            ...
            bot.send_message(OWNER_ID, f"📤 ꜰɪʟᴇ '{file_name}' ᴜᴘʟᴏᴀᴅᴇᴅ ʙʏ {message.from_user.first_name} (`{user_id}`)", parse_mode='Markdown')
        except Exception as e:
...
def check_subscription_init_callback(call):
    bot.answer_callback_query(call.id)
    msg = bot.send_message(call.message.chat.id, "🎫 ꜱᴇɴᴅ ᴜꜱᴇʀ ɪᴅ ᴛᴏ ᴄʜᴇᴄᴋ ꜱᴜʙꜱᴄʀɪᴘᴛɪᴏɴ.\n/ᴄᴀɴᴄᴇʟ ᴛᴏ ᴄᴀɴᴄᴇʟ.")
...
Remember: Always keep backups of your bots!"""
